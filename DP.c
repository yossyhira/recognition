...
int main(void) {
    ...
    for (int tmp_Idx = 0; tmp_Idx < ALLFILE; tmp_Idx++) {
        ...
        for (int unknown_Idx = 0; unknown_Idx < ALLFILE; unknown_Idx++) {
            ...
            T_min_Idx = (T_min == T[unknown_Idx]) ? unknown_Idx : T_min_Idx;//最小の単語間距離の未知の単語のファイル番号(インデックス)
        }

        if (T_min_Idx == tmp_Idx) {
            count++;//一致していれば次に進む
        } else {
            printf("以下のテンプレートの音声データと未知の音声データが一致しているという誤認識をしました。\n");
            printf("テンプレートの音声データ : city%03d/city%03d_%03d.txt\n", tmp, tmp, tmp_Idx + 1);
            printf("発声内容 : %s",city_tmp.word);
            printf("未知の音声データ : city%03d/city%03d_%03d.txt\n", unknown, unknown, T_min_Idx + 1);
            sprintf(ch, "/home/hirata/recognition/city_mcepdata/city%.3d/city%.3d_%.3d.txt", unknown, unknown, T_min_Idx + 1);
            //ファイルを読み込みモードで開く
            fp_unknown = fopen(ch, "r");
            //ファイル読み取りエラー処理
            if (fp_unknown == NULL) {
